...
实测显示公式：
- 步数显示 = moveCount - 8 (如果 > 8) 或 moveCount (如果 ≤ 8)
- 目标显示 = m_ct + 12 (如果 m_ct ≥ 8) 或 m_ct + 28 (如果 m_ct < 8)

实测数据验证：
- 配置：步数30，目标1 → 显示：步数22，目标29 ✓
...
def calculate_display_values(config_steps, config_targets):
    """根据实测公式计算显示值"""
    # 步数显示逻辑
    display_steps = config_steps - 8 if config_steps > 8 else config_steps
    
    # 目标显示逻辑
    display_targets = []
    for target in config_targets:
        if target >= 8:
            display_targets.append(target + 12)
        else:
            display_targets.append(target + 28)
...
def reverse_calculate_config(target_display_steps, target_display_targets):
    """根据目标显示值反推配置值"""
    # 步数配置：显示值 + 8
    config_steps = target_display_steps + 8
    ...
    for display_target in target_display_targets:
        # 尝试两种情况，选择合理的一种
        # 情况1: 配置目标 ≥ 8, 显示 = 配置 + 12
        config1 = display_target - 12
        # 情况2: 配置目标 < 8, 显示 = 配置 + 28  
        config2 = display_target - 28
        
        # 优先选择合理的配置值
        if config1 >= 8:
            config_targets.append(config1)
        elif config2 >= 0 and config2 < 8:
            config_targets.append(config2)
        else:
            # 如果都不合理，选择非负数的
            config_targets.append(max(0, config1))
...
def main():
    ...
    print("=== 基于实测数据的正确批量调整 ===")
    print("显示公式（实测验证）：")
    print("  步数显示 = moveCount - 8 (if > 8) else moveCount")
    print("  目标显示 = m_ct + 12 (if ≥ 8) else m_ct + 28")
    print("实测验证：配置[30,1] → 显示[22,29] ✓")
    print()
