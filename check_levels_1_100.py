...
def check_levels_1_100():
    """Check levels 1-100 for display values > 3"""
    config_dir = "assets/resources/config"
    levels_to_adjust = []
    
    print("Checking levels 1-100 for display values > 3...")
    print("Level | Steps | Targets | Needs Adjustment")
    print("-" * 45)
    ...
    for level in range(1, 101):
        ...
        try:
            ...
            needs_adjustment = display_steps > 3 or display_targets > 3
            
            print(f"{level:5} | {display_steps:5} | {display_targets:7} | {'YES' if needs_adjustment else 'NO'}")
    ...
    print(f"\nFound {len(levels_to_adjust)} levels that need adjustment")
    return levels_to_adjust

if __name__ == "__main__":
    levels_to_adjust = check_levels_1_100()
    
    if levels_to_adjust:
        print("\nLevels that need adjustment (display values > 3):")
        for level_info in levels_to_adjust:
            print(f"Level {level_info['level']}: Steps={level_info['current_steps']}, Targets={level_info['current_targets']}")
    else:
        print("\nNo levels found with display values > 3")